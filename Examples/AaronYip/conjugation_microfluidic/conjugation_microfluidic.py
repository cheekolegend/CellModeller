...
gfp_intensities = {'donor':0, 'recip':1, 'trans':0.99} #RGB cell colours
...
conj_freq = 0.1 # conjugation per contact per hour
...
def update(sim, cells):
    for (id, cell) in cells.items():
        ...
        # Conjugation
        if cell.cellType == 'donor' or cell.cellType == 'trans': #if a donor
            for neigh in cell.neighbours: #loop through all contacts
                recip = cells[neigh]
                rand_num = random.random() 
                if recip.cellType == 'recip': #if a recipient
                    if rand_num < conj_prob:
                        recip.cellType = 'trans' #become transconjugant
                ...
                # Exit the neighbour search loop if conjugation occurred      
                if rand_num < conj_prob:
                    break
