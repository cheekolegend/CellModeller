...
conj_freq = 10 # conjugation per contact per hour
conj_prob = conj_freq*dt # probability on a per-time-step basis

delay_mean = 60/60 # delay between subsequent conjugations (h)
delay_std = 0.1*delay_mean
...
def update(sim, cells):
    for (id, cell) in cells.items():
        ...
        # Conjugation
        conjugation_modules.conjugation_delay(sim, cells, cell, conj_freq, dt, rfp_intensities, colors, delay_mean, delay_std)
